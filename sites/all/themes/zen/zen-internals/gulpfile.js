...
// apteka's Sass files. It's a copy of apteka/gulpfile.s with a
...
// Define the node-scss configuration. The includePaths is critical!
options.sass = {
  importer: importOnce,
  includePaths: [
    options.theme.sass,
    options.rootPath.project + 'node_modules/breakpoint-scss/stylesheets',
    options.rootPath.project + 'node_modules/chroma-scss/scss',
    options.rootPath.project + 'node_modules/support-for/scss',
    options.rootPath.project + 'node_modules/typey/stylesheets',
    options.rootPath.project + 'node_modules/zen-grids/scss'
  ],
...
var gulp      = require('gulp'),
  ...
  del         = require('del'),
  // gulp-load-plugins will report "undefined" error unless you load gulp-scss manually.
...
gulp.task('lint:scss-with-fail', function() {
  return gulp.src(options.theme.sass + '**/*.scss')
    .pipe($.sassLint())
    .pipe($.sassLint.format())
...
gulp.task('watch:lint-and-styleguide', ['styleguide', 'lint:sass'], function() {
  return gulp.watch([
    ...
    ], options.gulpWatchOptions, ['styleguide', 'lint:scss']);
});
...
options.sass.includePaths.unshift(path.resolve('../apteka') + '/scss');
